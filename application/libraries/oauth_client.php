...
*/
	...
	<value></value>
	<documentation>
	<purpose>some oauth 2.0 site need header</purpose>
	<usage>http header</usage>
	</documentation>
	</variable>
	{/metadocument}
	*/
...
*/
	...
	Function OutputDebug($message)
	{
		...
		if($this->debug)
		{
			...
			//error_log($message);
			file_put_contents('log.txt',$this->debug_output,FILE_APPEND);
		}
		return(true);
	}
	...
	{
		if($this->ci->session->userdata('OAUTH_STATE'))
			$state = $this->ci->session->userdata('OAUTH_STATE');
		else
		{
			$state = time().'-'.substr(md5(rand().time()), 0, 6);
			$this->ci->session->set_userdata('OAUTH_STATE',$state);
		}
...
*/
	...
	{
		$this->ci->session->set_userdata(md5($this->access_token_url),$access_token);
...
*/
	...
	{
		if($this->ci->session->userdata(md5($this->access_token_url)))
			$access_token = $this->ci->session->userdata(md5($this->access_token_url));
		else
			$access_token = array();
		return true;
	}
...
*/
	Function ResetAccessToken()
	{
		...
		if($this->ci->session->userdata(md5($this->access_token_url)))
			$this->ci->session->unset_userdata(md5($this->access_token_url));
...
*/
	...
	Function SendAPIRequest($url, $method, $parameters, $oauth, $options, &$response)
	{
		...
		else if(strlen($this->headerKey))
		{
			$this->GetAccessToken($access_token);
			$arguments['Headers']['Authorization'] = $this->headerKey.' '.$access_token['value'];
		}
		
		if(strlen($error = $this->ci->http->SendRequest($arguments))
		|| strlen($error = $this->ci->http->ReadReplyHeaders($headers)))
		{
			$this->ci->http->Close();
	...
	Function Initialize()
	{
		switch($this->server)
		{
			...
			case '163':
				$this->oauth_version = '2.0';
				$this->request_token_url = '';
				$this->dialog_url = 'https://api.t.163.com/oauth2/authorize?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&state={STATE}';
				$this->append_state_to_redirect_uri = '';
				$this->access_token_url = 'https://api.t.163.com/oauth2/access_token';
				$this->authorization_header = true;
				$this->url_parameters = false;
				break;
			case '360':
				$this->oauth_version = '2.0';
				$this->request_token_url = '';
				$this->dialog_url = 'https://openapi.360.cn/oauth2/authorize?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&state={STATE}&scope={SCOPE}';
				$this->append_state_to_redirect_uri = '';
				$this->access_token_url = 'https://openapi.360.cn/oauth2/access_token';
				$this->authorization_header = true;
				$this->url_parameters = false;
				break;
			case 'douban':
				$this->oauth_version = '2.0';
				$this->request_token_url = '';
				$this->dialog_url = 'https://www.douban.com/service/auth2/auth?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&state={STATE}&scope={SCOPE}';
				$this->append_state_to_redirect_uri = '';
				$this->access_token_url = 'https://www.douban.com/service/auth2/token';
				$this->authorization_header = true;
				$this->url_parameters = false;
				$this->headerKey = 'Bearer';
				$this->content_type = 'application/json';
				break;
			case 'qq':
				$this->oauth_version = '2.0';
				$this->request_token_url = '';
				$this->dialog_url = 'https://open.t.qq.com/cgi-bin/oauth2/authorize?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&state={STATE}';
				$this->append_state_to_redirect_uri = '';
				$this->access_token_url = 'https://open.t.qq.com/cgi-bin/oauth2/access_token';
				$this->authorization_header = true;
				$this->url_parameters = true;
				break;
			case 'renren':
				$this->oauth_version = '2.0';
				$this->request_token_url = '';
				$this->dialog_url = 'https://graph.renren.com/oauth/authorize?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&state={STATE}';
				$this->append_state_to_redirect_uri = '';
			...
			case 'weibo':
				$this->oauth_version = '2.0';
				$this->request_token_url = '';
				$this->dialog_url = 'https://api.weibo.com/oauth2/authorize?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&state={STATE}';
				$this->append_state_to_redirect_uri = '';
			...
			case 'fetion':
				$this->oauth_version = '2.0';
				$this->request_token_url = '';
				$this->dialog_url = 'https://i.feixin.10086.cn/oauth2/authorize?client_id={CLIENT_ID}&response_type=code&redirect_uri={REDIRECT_URI}&state={STATE}&scope={SCOPE}&display=page';
				$this->append_state_to_redirect_uri = '';
				$this->access_token_url = 'https://i.feixin.10086.cn/oauth2/access_token';
				$this->authorization_header = true;
				$this->url_parameters = false;
				break;
	...
	Function Process()
	{
		switch(intval($this->oauth_version))
		{
			...
			case 2:
				...
				{
					$this->OutputDebug('access_token value isset:'.$access_token['value']);
				...
				if(!$this->GetStoredState($stored_state))
					return false;
				$this->OutputDebug('GetStoredState, value:'.$stored_state);
				
				if(strlen($stored_state) == 0)
					return($this->SetError('it was not set the OAuth state'));
				if(!$this->GetRequestState($state))
					return false;
				$this->OutputDebug('GetRequestState, value:'.$state);
				
				if($state === $stored_state)
				{
					//get code from response and request for access token
					$this->OutputDebug('state === $stored_state');
					if($this->debug)
